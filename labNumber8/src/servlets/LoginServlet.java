...
// восстановить имя через cookie
        ...
        // Вывести форму
        pw.println("<form action='/labNumber8/' method='post'>Введите имя:" +
...
// Иначе вместо символов будет абракадабра
        ...
        if (errorMessage!=null) {
            ...
            response.sendRedirect(response.encodeRedirectURL("/labNumber8/"));
        }
    }
...
// от множества пользователей
        ...
        if (aUser.getSessionId().equals(sessionId) ||
            ...
            response.sendRedirect(response.encodeRedirectURL("/labNumber8/view.html"));
